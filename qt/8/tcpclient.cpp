...
*/
QDataStream &operator >>(QDataStream &in, ServiceHeader &head){
...
QDataStream &operator <<(QDataStream &out, ServiceHeader &head){
...
QDataStream &operator >>(QDataStream &in, StatServer &stats){
...
{
    uint32_t payloadLen = str.toUtf8().size() + sizeof(int);
    head.len = payloadLen;
