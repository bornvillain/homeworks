...
public:
    ...
    virtual std::string printAsHTML() const = 0;  //interface segregation principle
...
public:
    ...
    std::string printAsHTML() const override
    {
        if (format_ != Format::kHTML) {
            throw std::runtime_error("Invalid format!");  //Liskov substitution principle
        }
...
void saveTo(std::ofstream &file, const Printable& printable, Data::Format format)
{
    switch (format)
    {
        case Data::Format::kText:               //open-closed principle
